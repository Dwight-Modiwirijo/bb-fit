#!/usr/bin/env python3
"""
Orchestrator tpsl_v5 — volledig automatisch, nul handmatige stappen nodig.

Fases:
  0. Wacht op v5_warmup/checkpoint_best.pt  (warmup loopt al)
  ...
  3. Rapport in orchestrator_v5_report.md na elke ronde
...
Usage:
  python orchestrate_tpsl_v5.py          # start normaal
  python orchestrate_tpsl_v5.py --skip-warmup-wait   # als warmup al klaar is
"""
import argparse
import json
import subprocess
...
REPORT_PATH    = DATA_HOST / "orchestrator_v5_report.md"

WARMUP_CKPT_HOST = CKPT_BASE_HOST / "v5_warmup" / "checkpoint_best.pt"
WARMUP_CKPT_CTR  = "/workspace/checkpoints/lstm_bbfit/v5_warmup/checkpoint_best.pt"

SEQ_DIR         = "/workspace/data/sequences_tpsl_v5"
FINETUNE_CSV    = DATA_HOST / "sequences_tpsl_v5" / "lstm_train_balanced_finetune.csv"

# ── Definition of Done ────────────────────────────────────────────────────────
DOD_LONG_PREC  = 0.75   # long precision >= 75%  (break-even is 69.2% bij TP=0.8%/SL=1.8%)
DOD_SHORT_PREC = 0.10   # short precision >= 10%  (bijzaak, exit via TP/SL)
DOD_LONG_REC   = 0.15   # long recall >= 15% (voorkom dat model stopt bij te weinig trades)
...
# Evaluate runs on CPU to avoid GB10 CUDA context conflicts after training
DOCKER_FLAGS_CPU = [
...
def build_finetune_csv() -> None:
    ...
    rc = run_docker(cmd, DATA_HOST / "v5_build_finetune.log")
    if rc != 0:
...
def run_finetune(round_num: int, start_ckpt_ctr: str, hold_weight: float, lr: float) -> Path:
    name     = f"v5_orch_r{round_num:02d}"
...
def run_evaluate(round_num: int, ckpt_ctr: str) -> Tuple[dict, Path]:
    ...
    time.sleep(15)  # allow GPU memory to fully release after finetune container exits
...
def write_report(rounds: List[dict], dod_met: bool, best_idx: int,
    ...
    if not dod_met and rounds:
        ...
        lines += ["---", "", "## Volgende stap", "",
                  ...
                  f"- long_prec:  {last['metrics']['long_prec']:.1%}  (doel: {DOD_LONG_PREC:.0%})",
                  f"- long_rec:   {last['metrics']['long_rec']:.1%}  (doel: {DOD_LONG_REC:.0%})",
                  f"- short_prec: {last['metrics']['short_prec']:.1%}  (doel: {DOD_SHORT_PREC:.0%})",
                  ""]
...
def main() -> None:
    ...
    log("=" * 60)
    log("Orchestrator tpsl_v5 gestart")
