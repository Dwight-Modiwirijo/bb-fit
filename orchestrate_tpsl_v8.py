#!/usr/bin/env python3
"""
Orchestrator tpsl_v8 — auto finetune loop for the v8 feature set.

v8 vs v7:
  - Input: lstm_merged_v8.csv (all rows incl. force_hold, for temporal context)
  - Sequence build uses --feature-set v8 (last-candle is_valid_signal filter)
  - Same 36 features (35 V8_NUMERIC + 1 interval_code)
  - Force-hold rows appear only in sequence context, never as targets
  - DoD unchanged: long_precision >= 75%

Phases:
  0. Wait for v8_warmup/checkpoint_best.pt
  1. Build balanced finetune CSV (1:1:1)
  2. Finetune -> evaluate -> DoD check -> repeat (max MAX_ROUNDS)

Usage:
  python orchestrate_tpsl_v8.py                    # normal start
  python orchestrate_tpsl_v8.py --skip-warmup-wait # if warmup already done
  python orchestrate_tpsl_v8.py --start-round 3 --start-checkpoint /workspace/checkpoints/.../checkpoint_epoch05.pt
"""
import argparse
import json
import subprocess
...
REPORT_PATH    = DATA_HOST / "orchestrator_v8_report.md"

WARMUP_CKPT_HOST = CKPT_BASE_HOST / "v8_warmup" / "checkpoint_best.pt"

SEQ_DIR      = "/workspace/data/sequences_tpsl_v8"
FINETUNE_CSV = DATA_HOST / "sequences_tpsl_v8" / "lstm_train_balanced_finetune.csv"
...
def last_epoch_checkpoint_ctr(ckpt_dir_host: Path) -> str:
    ...
    Warmup checkpoint_best.pt is nearly random: validation set is 97% Hold,
    so the model is rewarded for predicting Hold. Always use last epoch.
    """
    checkpoints = sorted(ckpt_dir_host.glob("checkpoint_epoch*.pt"), key=lambda p: p.stat().st_mtime)
    if not checkpoints:
...
def build_finetune_csv() -> None:
    ...
    rc = run_docker(cmd, DATA_HOST / "v8_build_finetune.log")
    if rc != 0:
...
def run_finetune(round_num: int, start_ckpt_ctr: str, hold_weight: float, lr: float) -> Path:
    name     = f"v8_orch_r{round_num:02d}"
...
def write_report(rounds: List[dict], dod_met: bool, best_idx: int,
    ...
    lines += ["", "---", "", "## Notes", "",
              ...
              "- **input_size** = 36 (35 V8_NUMERIC_FEATURES + 1 interval_code)",
              "- **v8 key change**: force_hold rows kept in sequence context; filtered from targets",
              ""]
...
def main() -> None:
    ...
    log("=" * 60)
    log("Orchestrator tpsl_v8 started")
    ...
    log("v8: force_hold rows in context, filtered from targets")
    log("=" * 60)
    ...
    if args.start_checkpoint:
        ...
        warmup_dir = CKPT_BASE_HOST / "v8_warmup"
