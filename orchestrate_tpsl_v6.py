#!/usr/bin/env python3
"""
Orchestrator tpsl_v6 — auto finetune loop for the v6 feature set.

v6 vs v5:
  - Input: lstm_merged_v6.csv (FiveMinutes only, real TAengine indicators)
  - Feature set: 30 normalised features (V6_NUMERIC_FEATURES + interval_code)
  - LSTM input_size must be 30 (was ~35 in v5)
  - DoD unchanged: long_precision >= 75%

Phases:
  0. Wait for v6_warmup/checkpoint_best.pt
  1. Build balanced finetune CSV (1:1:1)
  2. Finetune -> evaluate -> DoD check -> repeat (max MAX_ROUNDS)

Usage:
  python orchestrate_tpsl_v6.py                    # normal start
  python orchestrate_tpsl_v6.py --skip-warmup-wait # if warmup already done
  python orchestrate_tpsl_v6.py --start-round 3 --start-checkpoint /workspace/checkpoints/.../checkpoint_epoch05.pt
"""
import argparse
import json
import subprocess
...
REPORT_PATH    = DATA_HOST / "orchestrator_v6_report.md"

WARMUP_CKPT_HOST = CKPT_BASE_HOST / "v6_warmup" / "checkpoint_best.pt"

SEQ_DIR      = "/workspace/data/sequences_tpsl_v6"
FINETUNE_CSV = DATA_HOST / "sequences_tpsl_v6" / "lstm_train_balanced_finetune.csv"
...
def build_finetune_csv() -> None:
    ...
    rc = run_docker(cmd, DATA_HOST / "v6_build_finetune.log")
    if rc != 0:
...
def run_finetune(round_num: int, start_ckpt_ctr: str, hold_weight: float, lr: float) -> Path:
    name     = f"v6_orch_r{round_num:02d}"
...
def main() -> None:
    ...
    if args.start_checkpoint:
        ...
        warmup_dir = CKPT_BASE_HOST / "v6_warmup"
